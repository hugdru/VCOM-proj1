...
void Circular_hough(int, void *)
{
    ...
    HoughCircles(src_gray, circles, HOUGH_GRADIENT,
                 ...
                 0, 0                  // change the last two parameters (min radius and max radios ) to detect large circles
                 );
    ...
    //cv::Mat roi( display, cv::Rect( center.x-radius, center.y-radius, radius*2, radius*2 ) );
    ...
    // Apply Canny edge detector (is requeired to first apply a edge detection algoritm)
    ...
    HoughLinesP(pre_processed_img, p_lines, 1, CV_PI / 180, min_threshold + p_trackbar, 30, 10);

    // 1º filtering the vector lines (selecting only the lines near the center)

    // 1º ver 2 pontos da linha perto do centro (eliminar resto)
    ...
    for (size_t x = 0; x < p_lines.size(); x++)
    {
        ...
        {
            // TODO: organize vector (center to border)
            clock_lines.push_back(l);
        }
    }

    // 2º ver angulo entre eles e depois juntar linhas com angulos parecidos

    // 2º calculating angle between them (5 degres for exemple or less)
    ...
    if (merged_clock_lines.size() > 1)
    {
        ...
        cout << "vec1: " << vec1 << ", vec2: " << vec2 << ",  ang: " << angleBetweenTwoLines(vec1, vec2) << endl;
...
{
    // Read the image
    String imageName("w.jpeg"); // by default
    if (argc > 1)
    {
        imageName = argv[1];
    }
    ...
    createTrackbar(cannyThresholdTrackbarName, windowName, &cannyThreshold, maxCannyThreshold, Circular_hough);
    createTrackbar(accumulatorThresholdTrackbarName, windowName, &accumulatorThreshold, maxAccumulatorThreshold, Circular_hough);
    char thresh_label[50];
    sprintf(thresh_label, "Thres: %d + input", min_threshold);
    createTrackbar(thresh_label, windowName, &p_trackbar, max_trackbar, Circular_hough);
